import { WebTranslations } from '#contents/web-translations';
import { create, enforce, only, test } from 'vest';
import 'vest/enforce/email';
import type * as model from './checkout.model';

export const suite = (translations: WebTranslations) =>
  create<keyof model.Customer>(
    (client: model.Customer, field: keyof model.Customer) => {
      only(field);
      ...
      test(
        'phonePrefix',
        translations['validations.phonePrefix.format'],
        () => {
